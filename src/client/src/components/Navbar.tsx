...
const navigation = [
    ...
    { name: 'Meine Rezepte', href: 'recipe', current: false },
    { name: 'Kategorien', href: '#', current: false },
    { name: <FontAwesomeIcon icon={ faSearch } />, href: '#', current: false },
    { name: 'Zufällige Empfehlung', href: '#', current: false },
]
                                        ...
                                        {navigation.map((item, index) => (
                                            <a
                                                ...
                                                className={classNames(
                                                    item.current ? 'text-munch-orange underline underline-offset-8' : 'text-white hover:border border-munch-orange hover:text-white',
                                        ...
                                        <MenuItems className="absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-black py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                                            <MenuItem>
                                                {({ focus }) => (
                                                    <a
                                                        href="/login"
                                                        className={classNames(focus ? 'text-munch-orange' : '', 'block px-4 py-2 text-sm text-white hover:text-munch-orange')}
                                                    >
                                                        Dein Profil
                                                    </a>
                                                )}
                        ...
                        <div className="space-y-1 px-2 pb-3 pt-2">
                            {navigation.map((item, index) => (
                                <DisclosureButton
                                    ...
                                    href={item.href}
                                    className={classNames(
                                        item.current ? 'text-munch-orange' : 'text-white hover:text-munch-orange',
